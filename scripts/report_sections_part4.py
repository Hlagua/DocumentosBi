...
def build_part4(doc):
    ...
    add_numbered(doc, 2, "Slope One mejoró significativamente la precisión de intensidad de consumo: En validación cruzada de 5 pliegues sobre 9,503 celdas activas, redujo el error absoluto medio (MAE) a 0.2593 ± 0.0052 (frente a 0.4063 de la media de usuario y 0.4606 de la media global, una mejora del 36.18%). En pruebas de ranking Top-N sobre los 3,653 clientes activos, empató en Hit-Rate@1 con la popularidad masiva (91.79% vs 91.29%, apenas 18 clientes de diferencia, Z = 0.7634, p = 0.4452), mientras que la popularidad pura superó ligeramente en Hit-Rate@2 (96.77% vs 94.36%) y MRR (0.9511 vs 0.9469). Esto confirma que en catálogos reducidos dominados por la tarjeta, el ordenamiento ingenuo por popularidad acierta con facilidad en Top-1; no obstante, la ventaja distintiva de Slope One reside en su capacidad para calibrar intensidades relativas de consumo sin degradar la precisión del primer producto sugerido.", "2. Precisión Predictiva de Slope One:")
    add_numbered(doc, 3, "El descarte cuantitativo de la co-ocurrencia transaccional cruda previno recomendaciones triviales: La comprobación empírica evidenció que calcular similitudes sobre frecuencias transaccionales sin transformar colapsa el espacio angular en un rango hiper-comprimido (0.9339 a 0.9610) con una varianza prácticamente nula (σ² = 0.00009), percibiendo un cobro de intereses como idéntico a un retiro en cajero. La migración hacia contratos de órdenes permanentes elevó la varianza a 0.0397 (un incremento de 450 veces), mientras que el Coseno Ajustado en transacciones alcanzó una varianza de 0.0942 (más de 1,000 veces superior), recuperando una señal de recomendación comercialmente útil para la entidad.", "3. Validez del Descarte Empírico:")
