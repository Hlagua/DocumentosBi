...
def build_part2(doc):
    ...
    add_bullet(doc, "Antisimetría estricta de las desviaciones: Para cualquier par de productos (j, i), se cumple que b(j, i) = -b(i, j), y para todo producto individual b(i, i) = 0. Esta propiedad reduce a la mitad el almacenamiento necesario de la matriz de desviaciones, almacenando únicamente el triángulo superior.", "1. Antisimetría:")
    add_bullet(doc, "Equivarianza frente a transformaciones afines: Si a todas las calificaciones de los usuarios se les aplica una transformación lineal positiva f(r) = α·r + β (con α > 0), las predicciones de Slope One escalan de manera idéntica: f(rhat) = α·rhat + β. Esto asegura robustez total frente a cambios en la escala de medición de los ratings implícitos.", "2. Invarianza de Escala:")
    add_bullet(doc, "Atenuación ponderada por soporte: A diferencia del promedio no ponderado, el término |S(j, i)| actúa como un factor de verosimilitud estadística bayesiana: las parejas de productos evaluadas por cientos de clientes comunes (como Tarjeta y Hogar con S = 3,365) dominan la predicción, mientras que pares con soporte reducido (como Préstamo y Seguro con S = 114) tienen una influencia proporcionalmente acotada.", "3. Ponderación por Soporte:")
    
    add_paragraph(doc, "Procedimiento metodológico y complejidad computacional:")
    add_numbered(doc, 1, "Fase de precomputación por lotes (Offline Batch): Sobre la matriz rala R de 3,653 clientes × 5 productos, se calculan las matrices densas simétricas de soporte |S(j, i)| y antisimétricas de desviación b(j, i). La complejidad temporal de esta fase es O(|U| · |I|²). Al tener |I| = 5, el procesamiento requiere apenas 25 evaluaciones por usuario, ejecutándose en fracciones de segundo en memoria RAM.", "Paso 1:")
    add_numbered(doc, 2, "Fase de inferencia en línea (Online Serving): Cuando un cliente u solicita recomendaciones en la banca móvil, el sistema recupera sus calificaciones conocidas R_u y aplica la fórmula ponderada de Slope One en tiempo O(|I|). La complejidad espacial es O(|I|²), ocupando un espacio despreciable en memoria.", "Paso 2:")
    add_numbered(doc, 3, "Filtrado de servicios activos: Se excluyen los productos que el cliente ya posee y se ordenan los productos candidatos de forma descendente según rhat_{u, j}.", "Paso 3:")
    ...
    # Modelo 1C
    add_heading(doc, 5, "Modelo 1C: Correlación de Pearson sobre Ratings y Series Mensuales Diferenciadas")
    add_paragraph(doc, "Fundamento teórico y justificación de primeras diferencias: Al evaluar la asociación lineal mediante Pearson sobre las calificaciones implícitas directas (Tabla 7-A), los coeficientes resultan artificialmente inflados (+0.4993 a +0.9957) debido a la positividad de la escala y la tendencia de crecimiento de la base de clientes. Para aislar la sincronización de tesorería mensual genuina y eliminar tendencias no estacionarias, las series de flujos mensuales agregados (72 meses) se transformaron mediante primeras diferencias (Δx_t = x_t - x_{t-1}, 71 observaciones):")
    ...
    add_paragraph(doc, "Un Lift de 1.12 confirma una convivencia armoniosa y positiva entre ambos servicios contractuales.")
